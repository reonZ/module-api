...
export function createFancyLink(docOrUuid, { async = true, label } = {}) {
    let link =
        docOrUuid instanceof foundry.abstract.Document ? docOrUuid.link : `@UUID[${docOrUuid}]`;
    if (label) {
        link = link.replace(/\{.+?\}$/, "");
        link = `${link}{${label}}`;
    }
 ...
 */
export async function createManipulationMessage(subtitle, content, actor, senderId) {
 ...
 * @returns {Promise<ChatMessage>}
 */
export async function createTradeMessage(subtitle, message, actor, item, senderId) {
